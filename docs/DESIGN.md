...
작성일: 2026-09-21 (5차 수정: 범위 확정)

> **범위**: 1차 구현은 썸네일·상세페이지 자동 생성(2.1~2.4, 2.5 의 ZIP 경로)이다.
> 이메일 발송(2.6)과 커머스 API 등록·연동(2.5 API 경로, 2.7)은 후순위로 두고 설계만 남긴다.
> 구현 현황은 README.md 를 본다.
...
## 2.7 운영 중인 스토어 연동 — 후순위

새 스토어가 아니라 이미 판매 중인 스토어에 붙이는 시스템이다. 브랜드와 상품을 새로 만들지 않고 이어받는다.

**기존 상품 가져오기.**
커머스 API 상품 조회로 등록된 상품 전체를 내려받아 `products/<sku>/` 구조로 옮긴다. 상품명, 가격, 옵션, 카테고리, 현재 썸네일·상세 이미지 URL 을 `meta.yaml` 과 `legacy/` 폴더에 보관한다. 원본 자료 파일은 스토어에 없으므로 판매자가 SKU 별로 넣는다. 이 매핑 작업이 단계 0 의 가장 큰 수작업이다.

**브랜드 스타일 이어받기.**
기존 썸네일과 상세 이미지 10~20장을 Claude 비전에 넣어 색상 팔레트, 폰트 느낌, 여백·구도, 배지 위치, 말투를 `brand_tokens.yaml` 로 뽑는다. 이것을 `templates/themes/` 초기값으로 쓰고, 판매자가 눈으로 보며 조정한다. 기존 상품과 신규 상품이 검색 결과에서 한 브랜드로 보여야 한다.

**적용 순서.**
1. 신규 상품부터 새 파이프라인으로 등록한다.
2. 기존 상품은 조회 대비 클릭률·판매가 낮은 것부터 재생성해 교체한다. 잘 팔리는 상품의 이미지는 건드리지 않는다.
3. 교체는 A/B 로 한다. 같은 상품의 썸네일을 바꾼 뒤 2주 클릭률을 비교하고 낮으면 되돌린다. 이전 이미지는 `legacy/` 에 남겨 즉시 복구할 수 있게 한다.

**발송 이관.**
- 현재 수동으로 보내는 방식(메일 계정, 안내 문구, 파일 보관 위치)을 그대로 `templates/email/` 과 발송 기록 초기 데이터로 옮긴다.
- 기존 상품 중 이메일 작성형 옵션이 없는 것은 옵션을 추가한다. 옵션 추가 전 주문은 구매자 요청사항·톡톡에서 이메일을 찾는 현재 방식이 남으므로 "보류" 목록으로 흘려 수동 처리한다.
- 전환 시점의 미발송 주문이 두 번 발송되지 않도록, 발송 기록에 과거 주문번호를 먼저 채운 뒤 폴링을 켠다.

**성과 데이터 활용.**
스마트스토어 센터 통계(상품별 조회·클릭·결제)를 주기적으로 내려받아 `analytics/` 에 쌓는다. 어떤 썸네일 목업·어떤 첫 화면 문구가 잘 되는지 시리즈 단위로 보고, 템플릿 기본값을 바꾸는 근거로 쓴다.
...
│   ├── store/
│   │   ├── import_catalog.py # 커머스 API 상품 조회 → products/
│   │   └── brand_tokens.py   # 기존 이미지 → brand_tokens.yaml
│   └── cli.py
├── templates/
│   ├── email/delivery.html
...
│   └── themes/subject_*.css
├── fonts/
├── products/<sku>/{source.*, meta.yaml, legacy/, pages/, out/}
├── analytics/
└── tests/
```
...
| 단계 | 기간 | 산출 | 완료 기준 |
|---|---|---|---|
| 0. 준비 | 1~2주 | 기존 상품 카탈로그 임포트와 원본 파일 매핑, 브랜드 토큰 추출, 썸네일 목업 3종·섹션 템플릿 8종, 스키마 확정, 커머스 API 앱 신청, 발신 도메인 SPF/DKIM | 기존 상품 1건을 새 템플릿으로 만들었을 때 기존 이미지와 같은 브랜드로 보인다 |
...
7. 운영 중인 스토어에 붙인다. 브랜드 룩은 기존 이미지에서 추출하고, 신규 상품부터 적용한 뒤 기존 상품은 성과가 낮은 것부터 A/B 로 교체한다.
